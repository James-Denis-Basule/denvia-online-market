...
export function BusinessProvider({ children }: BusinessProviderProps) {
  ...
  const resolveActiveBusiness = useCallback(
    (availableBusinesses: Business[]) => {
      ...
      const resolved =
        selectedFromServer ?? storedBusiness ?? userBusiness ?? availableBusinesses[0];
  ...
  const selectActiveBusiness = useCallback(
    async (businessId: string) => {
      ...
      persistBusinessId(targetBusiness._id);

      try {
        const response = await selectBusiness(businessId);
        const selectedBusiness = response.data?.business ?? targetBusiness;
