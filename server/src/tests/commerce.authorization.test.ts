...
function authorizationFor(user: { _id: mongoose.Types.ObjectId; role: string }) {
...
test('commerce operations enforce customer ownership and seller business ownership', async () => {
  ...
  try {
    const [customerA, customerB, sellerA, sellerB] = await Promise.all([
      User.create({ firstName: 'Customer', lastName: 'One', email: 'customer-one@example.com', password: 'secret123', role: 'user' }),
      User.create({ firstName: 'Customer', lastName: 'Two', email: 'customer-two@example.com', password: 'secret123', role: 'user' }),
      User.create({ firstName: 'Seller', lastName: 'One', email: 'seller-one@example.com', password: 'secret123', role: 'business_owner' }),
      User.create({ firstName: 'Seller', lastName: 'Two', email: 'seller-two@example.com', password: 'secret123', role: 'business_owner' }),
    ]);

    const [businessA, businessB] = await Promise.all([
      Business.create({ ownerId: sellerA._id, name: 'Seller One Store', slug: 'seller-one-store', email: 'seller-one-store@example.com' }),
      Business.create({ ownerId: sellerB._id, name: 'Seller Two Store', slug: 'seller-two-store', email: 'seller-two-store@example.com' }),
    ]);

    const createOrder = (userId: mongoose.Types.ObjectId, businessId: mongoose.Types.ObjectId) => Order.create({
      userId,
      items: [{
    ...
    const [orderA, orderB, mixedBusinessOrder] = await Promise.all([
      ...
      Order.create({
        ...
        items: [
          { productId: new mongoose.Types.ObjectId(), businessId: businessA._id, name: 'Seller A product', price: 10000, currency: 'UGX', quantity: 1 },
          { productId: new mongoose.Types.ObjectId(), businessId: businessB._id, name: 'Seller B product', price: 10000, currency: 'UGX', quantity: 1 },
        ],
        subtotal: 20000,
        deliveryFee: 5000,
        paymentFee: 0,
    ...
    const jsonRequest = (method: string, token: string, body?: Record<string, unknown>) => ({
      method,
      headers: { Authorization: token, 'Content-Type': 'application/json' },
      ...(body ? { body: JSON.stringify(body) } : {}),
    });

    const otherCustomerOrder = await request(`/api/marketplace/orders/${orderB._id}`, {
      headers: { Authorization: authorizationFor(customerA) },
    });
    assert.equal(otherCustomerOrder.status, 403);

    const otherSellerOrders = await request(`/api/marketplace/orders/seller?businessIds=${businessB._id}`, {
      headers: { Authorization: authorizationFor(sellerA) },
    });
    ...
    const unauthenticatedAssignment = await request(`/api/marketplace/orders/${orderA._id}/assign-delivery`, {
      method: 'POST',
    ...
    const customerAssignment = await request(
      ...
      jsonRequest('POST', authorizationFor(customerA), { courier: 'local_dispatch' }),
    );
    ...
    const customerOwnOrder = await request(`/api/marketplace/orders/${orderA._id}`, {
      headers: { Authorization: authorizationFor(customerA) },
    });
    assert.equal(customerOwnOrder.status, 200);

    const customerOwnDelivery = await request(`/api/marketplace/orders/${orderA._id}/delivery`, {
      headers: { Authorization: authorizationFor(customerA) },
    });
    assert.equal(customerOwnDelivery.status, 200);

    const sellerOwnOrders = await request(`/api/marketplace/orders/seller?businessIds=${businessA._id}`, {
      headers: { Authorization: authorizationFor(sellerA) },
    });
    ...
    const sellerAssignment = await request(
      `/api/marketplace/orders/${orderA._id}/assign-delivery`,
      jsonRequest('POST', authorizationFor(sellerA), { courier: 'local_dispatch', trackingCode: 'AUTH-1' }),
    );
