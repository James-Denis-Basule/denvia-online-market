...
export async function cancelOrderForUserOrAuthorizedSeller(
  ...
  // Authorization is performed against the authoritative order.
  const authorizedOrder = await getOrderForCustomerOrAuthorizedSeller(
    actor,
    orderId,
  );

  // Re-fetch as a Mongoose document because cancellation mutates and saves it.
  const order = await Order.findById(authorizedOrder._id);
  ...
  /*
   * Restore inventory from the authoritative order items.
   *
   * We only restore stock while the order is moving from an active
   * state to cancelled. A second cancellation is rejected above,
   * preventing the same stock from being restored twice.
   */
  for (const item of order.items) {
    ...
    if (result.matchedCount !== 1) {
      throw new AppError(
        `Product ${String(item.productId)} could not be restored`,
        409,
      );
    }
  }
  ...
  /*
   * Refund only a successfully paid payment.
   *
  ...
  /*
   * A delivery that has not started transit cannot continue after
   * cancellation. Delivery has no dedicated "cancelled" state in
  ...
  if (
    ...
    (delivery as any).events = Array.isArray((delivery as any).events)
      ? (delivery as any).events
      : [];

    (delivery as any).events.push({
      status: "failed",
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const authoritativeItems =
        await resolveAuthoritativeCartItems(sourceItems);
       ...
       *
       * If any reservation fails, the transaction aborts and
       * every previous stock reservation is automatically restored.
       */
      ...
      const normalizedShippingMethod = normalizeDeliveryMethod(
